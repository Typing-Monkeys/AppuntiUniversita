...
\begin{itemize}
    \item \textbf{Combinazione (+):} dove metto insieme tutti gli assegnamenti,
          combinando i vincoli, quindi si calcola anche il costo totale ad esempio con
          l'operazione di somma;
    \item \textbf{Proiezione ($\times$):} operazione di scelta della soluzione
          migliore data dalla combinazione in base al minimo o al massimo del costo.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{+:} operatore di proiezione, usato per fare la scelta fra le
          ...
          \begin{itemize}
              \item \textbf{idempotente:} se faccio a+a dove +=minimo il
                    risultato è sempre a, quando un operatore è idempotente è
                    possibile definire un ordinamento ovvero
                    \begin{center}
                        $a <= b$ dove b è meglio di a $\Leftarrow \Rightarrow$
                        $a + b = b$
                    \end{center}
          \end{itemize}
    \item \textbf{$\times$:} operatore di combinazione, utilizzato per combinare
          i vincoli, può essere somma,moltiplicazione etc... dipende dal problema.
          Possiamo definire alcune proprietà:
          \begin{itemize}
...
l'operatore $+$ non si ci si riferisce alla somma ma a un operatore per la
...
\begin{itemize}
    \item \textbf{Probabilistico:} $< R^+ , min, +, +\infty, 0>$ si massimizza
          la probabilità
    \item \textbf{Weighted:} $<[0, 1], max,\times,0,1>$ massimo il costo dato
          dalla combinazione, dove si fa il prodotto;
...
Il vincolo in mezzo è binario, se è $< a, a >$ significa "quando X$=$a e Y$=$a"
(perché è tra le variabili x ed y).
