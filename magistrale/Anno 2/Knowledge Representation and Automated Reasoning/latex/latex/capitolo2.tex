\chapter{Costraint Satisfaction Problems} \label{ch:Costraint Satisfaction
    Problems}
...
\begin{center}
    $\{(rosso, verde), (rosso, blu), (verde, rosso), (verde, blu), (blu, rosso),
        (blu, verde)\}$
\end{center}
...
\begin{enumerate}
    ...
    \item \textbf{Monotonicità:} Appena un vincolo viene violato posso
          interrompere la ricerca perchè sono sicuro che non esisterà soluzione.
...
\begin{enumerate}
    ...
    \item \textbf{Backtraking:} Assegno uno alla volta i valori alle variabili
          finchè non violo un vincolo; appena lo violo, torno indietro
...
nel momento in cui il variabile viene scelta da SELECT-UNASSIGNED-VARIABLE. Ma
...
questo assegnamento mi renda possibile futuri assegnamenti ad altri nodi, poichè
...
fatto un assegnamento ad una variabile, \textbf{se un dominio diventa vuoto,
    interrompi la ricerca poichè la soluzione non esiste con gli assegnamenti
...
questo tipo di assegnamento non pu`o andare bene.
...
possibile perchè sono due zone vicine. Il forward checking non rileva questo
...
\begin{itemize}
    \item \textbf{Consistenza di Nodo:} Consiste nella verifica dei valori del
          ...
          dominio del nodo $x_1$ è pari a 1,2,3 e il vincolo unario di $x_1$ è $x_1 >
              1$, la consistenza di nodo consente di ridurre il dominio di x1 a 2,3
    ...
    \item \textbf{Consistenza di Arco:} Consiste nella verifica dei valori di un
          ...
          vincolo $x_2 = x_{1}^2$ consente di ridurre il dominio del nodo x2 a 1,4,9
          poichè non sussiste alcun nesso tra il valore 5 dell'insieme dominio di
    ...
    \item \textbf{Consistenza di Cammino:} Consiste nella verifica dei valori di
          ...
          cammino è un estenzione della consistenza di arco ed è detta anche
...
dominio (domain consistent) se nessun valore del dominio del nodo è è dichiarato
...
Quando tutti i vincoli sono binari si parla di arc consistency. Un arco (u,c) è
...
\noindent \textbf{Conclusione:} In questo caso sono stato molto fortunato perchè
...
vado ad esplorare è esponenziale perchè ad X dovrei moltiplicare tutti i
...
dom(u) che sono incosistenti con tutti i valori in dom(v). \\Restituisce true se
...
Un singolo passo dell'agoritmo REVISE non è sufficiente. L'algoritmo base per
arc consistecy è AC-1, il quale esegue l'algoritmo REVISE finchè il dominio
...
non è sempre necessario, perchè la modifica del dominio di una variabile
influenza solamente le variabili che sono collegate a X da un vincolo e non le
restanti.\\
La complessità temporale è $O(d^3$ * $n$ * $e)$ e quella spaziale è $O(e$ * $n$
* $d)$.

\subsubsection{Algoritmo AC-2}
AC-2 è un algoritmo che può fare arc consistecy in un solo passo attraverso i
...
\begin{itemize}
    ...
    \item Ogni volta che qualsiasi valore è cancellato da un arco, guarda ai
          ...
          Se può essere eliminato, si continua il processo iterativamente finchè non
...
AC-3 è un miglioramento di AC-2, alcuni di essi possono essere già essere nella
...
\end{figure}
\\Ad esempio, 1 ha supporto in X perchè in Y c'è 2 e 3. Se un elemento non ha
...
\end{figure}
\\No, perchè il 2 andrebbe eliminato da Z. Se riesco a buttare via tutti gli
    ...
    \item \textbf{+:} operatore di proiezione, usato per fare la scelta fra le
          ...
          \begin{itemize}
              \item \textbf{idempotente:} se faccio a+a dove l'operatore + è il
                    ...
                    \begin{center}
                        $a <= b$ dove b è meglio di a $\Leftarrow \Rightarrow$
                        $a + b = b$
                    \end{center}
          \end{itemize}
...
\begin{itemize}
    \item \textbf{Probabilistico:} $< R^+ , min, +, +\infty, 0>$ si massimizza
          la probabilità
    \item \textbf{Weighted:} $<[0, 1] max,x,0,1>$ massimo il costo dato dalla
          combinazione, dove si fa il prodotto;
    \item \textbf{Fuzzy:} $<[0,1],max,min,0,1>;$
    \item \textbf{Classical:} $<{false,true},$ $\lor$ (OR), $\land$(AND)$,
              false, true>$
\end{itemize}
...
\begin{enumerate}
    \item Dato che l'operazione x di combinazione è la moltiplicazione faccio:
          ...
          su c2 e 0.9 i tre valori su c3 (secondo me).
...
vincoli più la soluzione mi migliora. Questo perchè più vincoliamo il problema e
...
soluzione migliora perchè si moltiplica di meno (meno calcoli da fare),se li
...
\begin{enumerate}
    \item \textbf{Combinazione:} Prendo tutte le possibili tuple, gli assegno un
          ...
          rosso e $x_3$ è blu è 0, perchè è il minimo tra 0 e 0.5. \\Il minimo
          se $x_2$ è rosso e $x_3$ è giallo è 0, perchè è il minimo tra 0 e 0.2.
          \\Continuo cosi per tutte le tuple. \\Non ho considerato le coppie di
          colore uguale perchè c'era scritto $pairwise-different$.
...
\begin{enumerate}
    \item Combinazione (Min): $c_1$ combinato $c_2$ ($c_3$ non lo prendo in
          ...
          \end{itemize}
          Facendo questo ho combinato i vincoli c1 e c2 . Per sapere poi quanto
...
a proiettare fa 0.8 perchè prendo il massimo. Se io vado a calcolare la
...
\begin{itemize}
    \item Idempotenza su \textbf{Combinazione}: Dipende strettamente dal
          semiring utilizzato:
          \begin{itemize}
              \item \textbf{Fuzzy:} Idempotente, perchè il minimo tra a ed a è
                    sempre a (il minimo tra due valori uguali da sempre lo stesso
                    valore).
              \item \textbf{Probabilistic:} Non idempotente, perchè a $*$ a non
                    fa esattamente a (ad esempio 0.1 $*$ 0.1 $=$ 0.01, la
                    moltiplicazione infatti non ha la proprietà di idempotenza).
              \item \textbf{Weighted:} Non idempotente, perchè a $+$ a non fa
                    esattamente a.
...
\end{itemize}
La distinzione tra operatore idempotente e non idempotente è importante perchè
...
Quanto vale c1 c2 proiettato x (su a)? devo fare la combinazione:
\begin{itemize}
    \item 0.9 $*$ 0.8 $=$ 0.72
    \item 0.9 $*$ 0.2 $=$ 0.18
    \item 0.9 $*$ 0.2 $=$ 0.18
...
posto di 0.9. Il problema però è che ho cambiato la soluzione finale, perchè la
...
\noindent L'operazione di local consistency nei CSP Crisp quindi classici è
importante perchè mantiene la soluzione del problema riducendo gli elementi del
dominio e non modificando le soluzioni.

\vspace{0.5cm}

\noindent Nel caso Soft invece, la soluzione non viene modificata solamente se
...
sarebbero accettabili perchè sbagliate.
...
\end{figure}
I nuovi valori per il vincolo c3 sono ottenuti analizzando prima y $=$ a e
...
divisione perchè era l'operazione inversa della moltiplicazione (che è
...
\begin{itemize}
    ...
    \item R $\subseteq$ A × A è una relazione rappresentante gli "attacchi"
          ("sconfitte")
\end{itemize}
\begin{center}
...
argomenti perchè attaccano gli altri e contrattaccano nel caso siano attaccati.
...
e anche i singoli argomenti tranne e poichè esso si contraddice da solo visto
che ha un cappio.
...
prendo d lui oltre che difendere se stesso da c (poichè attaccato) difende anche
...
b, (c, b) $\in$ R. Il sottoinsieme \{b,d\} non viene scelto poichè d è attaccato
da c ma a si difende a sua volta contrattaccando, mentre b è attaccato sia da a
che c, nel primo caso nessuno lo difende nel secondo d difende b perchè attacca
c.
\begin{itemize}
...
gli argomenti che sono difesi. \\Insieme V uoto è \textbf{Completo}? Va messo
...
perchè a è sempre difeso. a è sempre difeso da tutti perchè non è attaccato da
    ...
    \item \{a, d\} è \textbf{completo?}
          \begin{itemize}
              \item a chi difende? attacca b quindi difenderebbe tutti quelli
                    attaccati da b, ma b non attacca nessuno quindi a difende solo se
                    stesso.
              \item d chi difende? se stesso, perchè viene attaccato da c ma si
          ...
          perchè non viene attaccato da nessuno, quindi è \textbf{come se
              venisse difeso da tutti quanti}, e lui non difende nessuno, poichè b
...
completi? \\no, perchè a lo è ma non c quindi non è grounded. \\a, d è
\textbf{grounded}? i suoi sottoinsiemi singoli sono a e d, questi sono completi?
\\no, perchè a lo è ma non d quindi non è grounded. \\a è \textbf{grounded}? Si,
...
perchè a attacca b e c attacca d e b ma nessuno dei due attacca e. Mentre
...
\noindent a, c \textbf{non è stabile} perchè è si ammissibile (b che sta fuori è
attaccato e ok), d sta fuori ed è attaccato, ma e non lo attacca ne a ne c
quindi questo insieme non può essere stabile. \\a, d \textbf{è stabile} perchè a
attacca b e d attacca c, e quindi tutti gli elementi fuori dall'insieme sono
attaccati. \\La semantica stabile è la più forte di tutte le estenzioni, perchè
sono le posizioni più forti in un dialogo, sono la scelta migliore quando
utilizzo AF come decision making. Il problema delle estenzioni stabili è che non
sempre esistono.
...
difenderebbe dall'attacco di D (perchè ricordiamo era una funzione booleana). In
questo caso però, essendo che d attacca c con 9 e c risponde con 8, c non potrà
difendersi dall'attacco, poichè la difesa non è sufficiente a contrastare
quest'ultimo. Questa nozione dipende strettamente dal semiring utilizzato,
poichè per ogni semiring (i cui tipi sono gli stessi introdotti precedentemente)
...
attacchi che arrivano a b cioè da tutti gli R(a, b). \\$>=_S$ è da intendere
    ...
    \begin{itemize}
        \item Con W (a, B $\cup$ \{b\}) si intende il costo degli attacchi che
              ...
              (proprio perchè l'operatore di combinazione è la somma).
...
W, S)$, a, b, c $\in$ $A_{rgs}$ , B $\subseteq$ $A_{rgs}$ , allora b è
    ...
    perchè si attaccato 100. (b, c) sono abbastanza compatibili, perchè la
    ...
    una cosa positiva, perchè si vorrebbero cercare delle coalizioni superiori
    ...
    molto basso. Introduciamo quindi l'Alpha-hamma consistenza.\\
    ...
    escluso dall'insieme perchè porta un attacco con peso molto alto all'insieme
    ...
    \textbf{N.B} secondo Bistarelli l'insieme B non è ammissibile, perchè
...
$\gamma$ che in questo caso è 3. \\Quindi:
    \begin{itemize}
        ...
        \item $\gamma$ Peso del \textbf{conflitto esterno} (in questo caso 3 (5
              - 2))
    \end{itemize}
    L'insieme dell'esempio sopra è $2^3$ \textbf{admissible} perchè $\alpha$ = 2
    e $\gamma$ = 3. Inoltre B è anche stabile, perchè tutti gli elementi esterni
    ...
    perchè nessuno attaccava d (a rimaneva comunque fuori). \\
...
bottom, top)$ e $\alpha, \gamma \in$ A:
    ...
    \\Se un estenzione è $ \alpha_1$ ammissibile e $\alpha_1 >=_s \alpha_2$
    allora quell'estenzione è anche $\alpha_2$ ammissibile. Stessa cosa per
$\gamma$. \\Questo significa che nell'esempio sopra che era $2^3$ admissible
    ...
    \end{center}
    Un estenzione E è grounded se e solo se è una complete e nessun altra
    estenzione dentro le complete è più piccola di lei (quindi è la più piccola
    delle complete).
    \begin{figure}[htp]
        \centering
    ...
    complete era (a, d, c) perchè a difendeva sia d che c insieme.
    \\\textbf{Calcolo delle complete con pesi (usiamo Bistarelli):} Le complete
    in questo caso sono a perchè non viene attaccata da nessuno e più tutte
    quelle difese da a cioè d e c. \textbf{Non le difende tutte e due insieme}
    perchè il costo 1 non è sufficiente e difenderle tutte e due, deve per forza
    ...
    l'insieme è sempre lo stesso delle complete, ma questo è un problema, perchè
    ...
    nessuno). \\Negli Weighted AF la definizione di Grounded va cambiata perchè
    ...
    \begin{itemize}
        ...
        \item \textbf{Skept} (In tutti gli insiemi): Se un elemento è
              \textbf{sempre} dentro un estenzione. In questo caso l'output sarà NO
              perchè non c'è un elemento che è comune a tutti gli insiemi. É possibile
    ...
    \begin{itemize}
        ...
        \item \textbf{Ammissible}: Richiede la nozione di difesa, cioè un
        ...
        \item \textbf{Complete}: Deve essere admissible, se un argomento è
              difeso questo è per forza dentro l'estenzione. Nell'esempio sopra a era
        ...
        \item \textbf{Prefered}: Si ottiene per inclusione insiemistica, cerco
              ...
              perchè si trova dentro \{a, c\},\{a, d\} che sono più grandi.
    ...
    esattamente alle semantiche basate su estenzioni, ed infatti possiamo
    ...
    \end{figure}
    \begin{itemize}
        \item $a$ è verde perchè non ha nessun altro argomento verde che lo
              attacca.
        \item $c$ è verde per lo stesso motivo
        \item $b$ e $d$ sono rossi perchè c'è almeno un argomento verde che li
              attacca.
    ...
    \end{figure}
    \\Questo labeling è admissible, ad esempio c è accettato perchè d che lo
    ...
    \end{figure}
    Questo esempio è comunque admissible, perchè non ho obblighi sull'argomento
    ...
    \end{figure}
    \\No, perchè l'argomento a è etichettato di giallo, ma la prima regola mi
    ...
    Di queste:
    \begin{enumerate}
        \item \textbf{Grounded} perchè solo l'argomento $a$ è IN. L'argomento
              $a$ dovrà essere in qualsiasi estenzione IN, proprio perchè non essendo
    ...
    \begin{itemize}
        \item $b:$ Ha valore 1 perchè la frase: ”non ha attaccanti” si traduce
              in $R^-_1(x) = 0$ quindi va nella prima opzione della funzione, si
              arresta subito e torna 1.
        \item $c:$ Non vale la prima opzione perchè è attaccato da b, quindi
    ...
    \end{itemize}
    In questo caso Cat(b)=1 perchè non ha attaccanti e Cat(c)=0.5 perchè è
...
$d^1_3?$
    ...
    attaccato da almeno due argomenti? \\NO, perchè a è attaccato da almeno 1
    ...
    \subsubsection{\textbf{Importante}} Si, perchè b è attaccato da almeno un
    ...
    argomenti, quindi proprio perchè b ha più difensori di a, lo preferisco.
    quindi:
    \begin{center}
        $b >^{pref}$ $a$
    \end{center}
    ...
    \begin{itemize}
        \item La funzione verde assegna agli argomenti il valore 1 se sono
              ...
              Se calcolo il valore per l'insieme a, c questo darà 1 perchè l'insieme è
              IN.
...
\begin{itemize}
    \item Lo shapley value di a rispetto agli argomenti IN è maggiore dello
          shapley value di b sempre rispetto agli argomenti IN.
    \item Oppure se il valore è uguale (quindi non mi basta andare a controllare
          solamente gli IN), a deve avere anche uno shapley value per gli OUT minore
          di quello di b.
\end{itemize}
\begin{center}
...
\begin{itemize}
    \item Lo shapley value per gli IN di a è esattamente uguale allo shapley
...
\begin{enumerate}
    ...
    \item Disegna un AF a caso, dimmi quali sono le estenzioni conflict-free,
