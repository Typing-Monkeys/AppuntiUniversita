...
arco. \\\textbf{Un esempio di un grafo che non ammette ciclo emiltoniano è un
    qualsiasi grafo avente almeno un nodo con grado uno.} Poichè arrivati in quel
...
memorizzati i numeri di porta, poichè in un grafo qualsiasi non conosciamo il
deg massimo.
...
\begin{enumerate}
    ...
    \item Infine $\forall v$, assegna arbitrariamente i numeri di porta da
          ...
          di porta, se esistono.% quindi i rimanenti archi non appartenenti a T, se
          esistono.
\end{enumerate}
...
\textbf{Perchè è $\Pi(n) \leq 4n$ il costo?} Questo equivale a chiedersi quanto
...
grafo è un \textbf{albero} siamo ottimi, poichè il lower bound è anch'esso
$n$.\\
...
applicare solo su grafi che siano Hamiltoniani (dotati di un cammino che tocca
tutti i nodi una e una sola volta), ma sapere se un grafo è Hamiltoniano è un
...
\begin{proof}
    ...
    \begin{proof}
            ...
            \item Base: $h(T_1) = 0 \Rightarrow v$ \textbf{è} una foglia in T.
                  \begin{itemize}
                      \item Se v è una foglia anche su G, allora l'automa lascia
                            ...
                            perchè $d=i$ quindi si va sullo stato N.
            ...
            \item Passo induttivo: $v$ \textbf{non} è una foglia in T.
                  \begin{itemize}
                      \item La porta "2" che è quella che l'automa attraverserà
                            ...
                            $h(T_2) \leq h(T_1) -1$, poichè si effettua uno step
        ...
        % Dove:
        % \begin{itemize}
        %     \item $\sum_{i=2}^{p} n_i = (n'-1)$ fa quel valore perchè sono tutti i
        %           nodi nei rispettivi sotto alberi, che vengono chiamati $n'$ -1 invece è
        %           perchè la sommatoria inizia da 2 e quel -1 è esattamente il nodo $v$.
        ...
        %           viceversa. P indica tutti i vicini di V e -1 poichè è l'arco in entrata
        %           "dall'alto".
        ...
        \item Caso 3: L'automa lascia il nodo $v$ attraverso l'arco $e$ di porta
              ...
              \begin{enumerate}
                  ...
                  \item L'arco $e$ non è in T; se così fosse all'altra estremità
                        ...
                        poichè siamo in stato B ed entrerà nel ciclo infinito.
                            [2 step]
              \end{enumerate}
...
volta e poi finchè il numero di porta resta quello non sbaglia più.
...
qualora ci fossero più strutture con la stessa etichetta, poichè potrò sempre
...
Poichè 3 sono il numero di nodi che creano un sottoalbero di tipo A, 2 $S_e$
indica il numero di nodi se l'albero è una foglia estesa e 2 $S_y$ poichè i nodi
gialli sono al più 2??? I nodi yellow che non sono diventati Orange è perchè
...
\vspace{-10mm}
\begin{itemize}
    \item $S_Y + S_O = 2 S_R - 2$ Poichè $S_Y + S_O$ sono intermedi ai nodi rossi,
...
$$ 2n - 2 + 2p < 2n -2 + 2 \frac{7}{8} n = 3.75n -2 $$ Si ha 2P poichè commetto
