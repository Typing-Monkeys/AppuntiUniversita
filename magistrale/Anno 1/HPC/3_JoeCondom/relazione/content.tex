...
Di seguito riporto il codice in C, compilato con \icode{gcc -o simple simple.c}:
...
\end{lstlisting}
\ \\
La keyword \icode{Executable} indica il file eseguibile che verr\`{a} avviato, \icode{Arguments} \`{e} la lista dei parametri da passare la programma, \icode{Log, Output ed Error} indicano rispettivamente dove verranno collocati i file di Log, Output e di Errore. \icode{Queue 2} indica che verranno eseguiti 2 job con il dato eseguibile e lista di parametri.\\
\ \\
...
Tramite il comando \icode{stress-ng --cpu 4} posso caricare di lavoro 4 CPU. %% rivedere sta merda
